...
- 컨테이너들이 서로 통신할 수 있도록 연결하는 **중앙 네트워크 허브 역할**을 합니다. 여러 컨테이너가 같은 네트워크 세그먼트에 속하게 만들어, 서로 통신할 수 있도록 합니다.
-	위치: **호스트 시스템에 존재**하며, 기본적으로 Docker가 시작될 때 자동으로 생성됩니다.
-	기능:
  - veth와 eth0 연결
	-	컨테이너들이 서로 같은 네트워크에서 통신하도록 연결합니다.
	-	호스트와 컨테이너 간의 트래픽을 처리합니다.
	-	IP 주소 할당: **docker0는 컨테이너에게 IP 주소를 할당**하며, 기본적으로 172.17.0.1/16와 같은 서브넷을 사용합니다.

### **veth (Virtual Ethernet)** 인터페이스
- veth는 **가상 이더넷 인터페이스**
- 컨테이너 생성됨과 동시에 컨테이너의 **eth0**과 대응되는 veth가 생성됨
- **컨테이너와 호스트 시스템 간의 네트워크 통신을 처리**하는 가상 네트워크 장치입니다. 
- veth는 항상 **쌍(pair)**으로 존재하며, 하나는 호스트 시스템에, 다른 하나는 컨테이너에 할당됩니다.
-	구조: veth 인터페이스는 두 개의 연결된 인터페이스로 이루어져 있습니다. 하나는 호스트 시스템의 네트워크 인터페이스(예: vethXXX), 다른 하나는 컨테이너의 네트워크 인터페이스(예: vethYYY)입니다.
-	기능:
	-	veth 인터페이스 쌍을 통해 컨테이너와 호스트 간의 네트워크 트래픽을 전달합니다.
	-	컨테이너는 veth 인터페이스를 사용하여 **docker0와 연결**되며, 호스트 시스템과의 네트워크 통신을 할 수 있습니다.
	-	컨테이너 간 통신은 veth 인터페이스를 통해 이루어집니다.
...
- 역할: host 네트워크 모드는 컨테이너가 **호스트 시스템의 네트워크 인터페이스를 그대로 사용**하도록 하는 모드입니다. 
- 이 모드에서는 컨테이너가 호스트 시스템의 네트워크를 공유하게 됩니다.
-	기능:
	-	컨테이너는 **고유의 IP 주소를 가지지 않고, 호스트 시스템의 네트워크를 사용**합니다.
	-	성능이 중요한 경우에 사용되며, 네트워크 오버헤드를 최소화할 수 있습니다.

### overlay (오버레이 네트워크)

- 역할: overlay 네트워크는 여러 Docker 호스트에 걸쳐 있는 컨테이너들 간의 통신을 가능하게 해주는 네트워크 드라이버입니다. 
- 주로 **Docker Swarm**과 같은 **오케스트레이션 환경에서 사용**됩니다.
-	기능:
	-	**여러 호스트에 걸쳐 있는 컨테이너들이 서로 연결**될 수 있도록 합니다.
	-	VXLAN(Virtual Extensible LAN) 기술을 사용하여, 물리적 네트워크를 가상화하고, 여러 호스트 간에 컨테이너 간 통신을 가능하게 합니다.

### macvlan (MACVLAN 네트워크)
- 역할: macvlan 네트워크는 컨테이너가 **물리적 네트워크와 같은 수준에서 동작**하도록 해주는 네트워크 모드입니다. 
- 각 컨테이너는 **고유한 MAC 주소**를 가지며, 실제 네트워크 상에서 독립된 장치처럼 동작합니다.
-	기능:
	-	컨테이너가 고유의 MAC 주소를 가지므로, 외부 네트워크에서 별도의 IP로 접근할 수 있습니다.
	-	보통 네트워크가 요구하는 고급 설정을 위해 사용됩니다.

### none (네트워크 없음)

- 역할: none 네트워크 모드는 컨테이너가 **네트워크 연결을 전혀 하지 않도록** 설정하는 모드입니다.
-	기능:
	-	컨테이너는 네트워크 인터페이스를 가지지 않으며, 외부와의 통신이 불가능합니다.
	-	주로 특수한 테스트나 격리된 환경에서 사용됩니다.

## 도커 볼륨

- 도커 컨테이너와 호스트 시스템 간에 데이터를 영구적으로 저장하고 공유할 수 있는 방법
- 도커는 컨테이너가 종료되거나 삭제될 때 컨테이너의 파일 시스템을 제거하기 때문에, 컨테이너 내에서 생성된 데이터는 기본적으로 **휘발성**
- 도커 볼륨을 사용하면, 데이터를 컨테이너의 수명과 관계없이 유지가능

### 도커 볼륨의 주요 특징:
1.	영속성 (Persistence):
도커 컨테이너가 삭제되더라도 볼륨에 저장된 데이터는 그대로 남아 있습니다. 이를 통해 데이터가 휘발되지 않도록 보장할 수 있습니다.
2.	컨테이너 간 데이터 공유:
**여러 컨테이너에서 동일한 볼륨**을 마운트하여 데이터를 공유하거나, 데이터를 유지하면서 컨테이너를 업데이트하는 방식으로 활용할 수 있습니다.
3.	독립성 (Isolation):
볼륨은 **컨테이너의 파일 시스템과 독립적으로 관리**되며, 호스트 시스템이나 다른 컨테이너와도 쉽게 연결할 수 있습니다.
4.	편리한 백업과 복구:
볼륨에 저장된 데이터는 도커 CLI 명령어(docker volume 명령)을 사용하여 백업하거나 복구할 수 있습니다.

### 볼륨 종류

- 호스트 볼륨
  - 도커 볼륨과는 달리, 호스트 파일 시스템의 경로를 직접 컨테이너에 마운트하여 호스트와 컨테이너 간에 데이터를 공유하거나 동기화
  - 호스트와 컨테이너 간의 파일 변경 사항이 실시간으로 반영
- 볼륨 컨테이너
  - 호스트 볼륨에 마운트 된 볼륨 전용 컨테이너를 생성해서 volumes-from으로 사용
- 도커 볼륨
  - 도커가 제공하는 볼륨 관리 기능 활용
  - 기본적으로 `/var/lib/docker/volumes/${volume-name}/_data` 에 데이터가 저장
