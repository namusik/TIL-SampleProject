...
metadata:      # Pod의 메타데이터를 정의. 오브젝트를 유일하게 식별하기 위한 정보
  ...
  labels:         # 생성되는 Pod에 실제로 부여되는 label
    ...
    - name: myapp-container # 컨테이너의 이름
      ...
      volumeMounts:
        ...
        - mountPath: /data/images
          # volumes에서 정의한 볼륨 이름
          name: image
      readinessProbe:  # 준비 상태 확인. 이 Pod이 트래픽을 받을 준비가 됐는지 체크
        failureThreshold: 3   #  3번 연속 실패하면 "준비 안됨" 판정
        successThreshold: 1   # 1번 성공하면 "준비됨" 판정
        httpGet:
          path: /actuator/health/readiness  # 이 URL로 확인
          port: 8080
          scheme: HTTP
        initialDelaySeconds: 120    # Pod 시작 후 120초 뒤에 첫 체크 시작
        periodSeconds: 5            # 5초마다 체크
      livenessProbe:          # 생존 상태 확인. 이 Pod이 살아있나? 죽었으면 재시작해야 하나?
        failureThreshold: 3   # 3번 연속 실패하면 "죽음" 판정
        successThreshold: 1   # 1번 성공하면 "살아있음" 판정
        httpGet:
          path: /actuator/health/liveness # 이 URL로 확인
          port: 8080
          scheme: HTTP
        initialDelaySeconds: 180      # Pod 시작 후 180초 뒤에 첫 체크 시작
        periodSeconds: 5              # 5초마다 체크
  volumes:      
    - name: spring-config
      configMap:
...
- initialDelaySeconds 설정 기준
  -  앱이 실제로 시작되는 시간 + 안전 마진(20~30%)
  -  Spring Boot 로그 예시
     -  `Started MmpApiApplication in 85.234 seconds`
  -  livenessProbe는 readinessProbe보다 길게
     -  readiness 통과 후에도 앱이 완전히 안정화되는 시간 필요
     -  liveness 실패 = Pod 재시작 (위험!)
     -  너무 빨리 체크하면 → 정상인데 재시작될 수 있음
...
spec:
  ...
  selector:
    matchLabels:
      app: mbp-api  # 이 레이블을 가진 파드를 Rollout이 관리 대상으로 삼음. template에 쓸 pod 중에서 "app: mbp-api"를 label로 가지는 Pod를 선택. 아래 template.metadate.labels에 있는 값이어야 함.
  template:
    metadata:
      annotations:
      ...
      labels:
        app: mbp-api  # Pod에 부여되는 label
    spec: 
  strategy:
    blueGreen:
      activeService: mbp-api               #  Service의 metadata.name과 일치해야됨. 현재 안정 버전(Blue)을 가리키는 Service 이름
      previewService: mbp-api-preview      # Service의 metadata.name과 일치해야됨. 새 버전(Green)을 가리키는 Service 이름
      autoPromotionEnabled: false              # false : 수동 promoteFull 필요 (현재 설정)
      scaleDownDelaySeconds: 30       # promoteFull 후 기존 Blue Pod 종료까지 대기 시간
      activeMetadata:
        labels:               # Blue(Active) Pod에 추가할 label
          service: blue
      previewMetadata:
        labels:              # Green(Preview) Pod에 추가할 label
          service: green
```
...
metadata:
  name: mbp-api   # 이 Service의 이름 (Ingress, Rollout에서 참조)
  namespace: mbp
  labels:
    app: mbp-api  # 이 Service 자체의 label (선택사항-관리용 (검색/필터))
spec:
  type: NodePort
  ports:                #  Service가 노출할 포트를 설정
    - port: 8080        # 외부에서 접근할 포트를 80으로 설정
      targetPort: 8080  #  내부에서 연결할 포트를 80으로 설정
  selector:       # 이 Service가 연결할 Pod의 label (Rollout에서 가져옴)
    app: mbp-api  # app: myapp 레이블을 가진 Pod을 선택. deployment/rollout에 있는 template 라벨과 일치해야 함.
...
metadata:   # Kubernetes 리소스(Ingress)를 식별/관리하기 위한 기본 식별자 + 라벨/주석(annotations) 을 담는 영역
  name: mbp-api-ingress   # Ingress 리소스의 이름
  namespace: mbp          # 이 Ingress가 속한 Kubernetes 네임스페이스
  annotations:      # Kubernetes 입장에서는 “주석(키-값 문자열)”이지만, AWS Load Balancer Controller 입장에서는 ALB 생성/설정의 선언적 파라미터
    kubernetes.io/ingress.class: alb        # 이 Ingress를 어느 Ingress Controller가 처리할지 지정. 값이 alb면 AWS Load Balancer Controller가 이 Ingress를 “자기 책임”으로 인식하고 ALB를 프로비저닝/리컨실
    alb.ingress.kubernetes.io/certificate-arn: arn:aws:acm:ap-northeast-2:854013278161:certificate/da2d1c66-fbc5-49ca-8222-20e3de6700cf   # ALB의 HTTPS 리스너에 연결할 ACM 인증서를 지정
    alb.ingress.kubernetes.io/scheme: internet-facing     # ALB의 노출 방식을 결정. internet-facing: 퍼블릭 서브넷에 생성되어 외부 인터넷에서 접근 가능. internal: 프라이빗 서브넷에 생성되어 VPC 내부 접근 전용
    alb.ingress.kubernetes.io/target-type: ip             # ALB Target Group이 무엇을 타겟으로 등록할지를 지정.  ip: Pod IP를 직접 타겟으로 등록. instance: 노드(EC2) 인스턴스를 타겟으로 등록(NodePort 필요)
    alb.ingress.kubernetes.io/listen-ports: '[{"HTTPS":8080}]'      # ALB에 생성할 리스너 목록(포트/프로토콜) 을 JSON 문자열로 선언
    alb.ingress.kubernetes.io/group.name: mbp-api                   # 여러 Ingress를 하나의 ALB로 묶어 공유하도록 하는 그룹 기능. 같은 group.name을 가진 Ingress들은 컨트롤러가 ALB는 1개로 만들고  룰/리스너 설정을 합산하여 구성
    alb.ingress.kubernetes.io/healthcheck-path: /actuator/health      # 컨트롤러가 생성하는 Target Group 헬스체크 경로. 이 경로로 ALB가 주기적으로 요청을 보내서 타겟이 정상인지 판정
    alb.ingress.kubernetes.io/wafv2-acl-arn: arn:aws:wafv2:ap-northeast-2:854013278161:regional/webacl/megabird-dev-alb-acl/c3c48bba-0d3c-4650-9beb-a5813798b3c2    # 생성된 ALB에 WAFv2 Web ACL을 연결

    # Green 테스트용: X-Deployment-Version: green 헤더로 preview 서비스 접근
    alb.ingress.kubernetes.io/conditions.dev-mbp-api-preview: |  # 특정 backend service(dev-mbp-api-preview)로 라우팅하는 조건(rule match 조건) 을 JSON으로 선언
      [
        {
          "field": "http-header",
          "httpHeaderConfig": {
...
spec:
  rules:      # 라우팅 규칙을 지정
    - http:
        paths:
          # 1. 헤더 조건: X-Deployment-Version: green → preview로 라우팅 (Green 테스트용)
          - path: /
            pathType: Prefix
            backend:
              service:
                name: dev-mbp-api-preview
                port:
                  number: 8080
          ...
          - path: /             # 루트 경로(/)에 대한 요청을 처리
            ...
            backend:
              service:
                name: dev-mbp-api   # Service의 metadata.name과 일치!
                port:
                  number: 8080
```
