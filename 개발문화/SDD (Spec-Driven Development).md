...
> 최종 업데이트: 2026-04-30 | GitHub Spec Kit / Claude Code·Cursor 등 AI 코딩 에이전트 시대 기준
...
```
[전통적 개발]
사람 → 코드 직접 작성 → 코드 자체가 진실
(스펙은 시간 지나면 안 맞아 "참고용"으로 전락)

[AI 에이전트 시대]
사람 → 명세 작성 → AI가 코드/테스트 생성 → 명세와 대조
(코드는 "재생성 가능한 산출물", 명세가 진실)
```
...
- **2025년** **GitHub가 "Spec Kit" 오픈소스 공개** — SDD를 가장 구체적으로 정의한 공식 워크플로우. Amazon이 Kiro를, 다른 벤더도 유사 프레임워크 발표
...
```
[/specify]   What — 무엇을 만들지 (사용자 시나리오, 요구사항)
     │
     ▼
[/clarify]   모호한 부분에 질문/답변으로 정밀화
     │
     ▼
[/plan]      How — 기술 계획 (스택, 아키텍처, 데이터 모델)
     │
     ▼
[/tasks]     실행 가능한 작업 분해 (체크리스트)
     │
     ▼
[/analyze]   일관성·누락 검증 (명세 ↔ 계획 ↔ 작업)
     │
     ▼
[/implement] AI 에이전트가 명세대로 구현
```
...
1. **API 단위로 시작** — 이미 API-First가 익숙하다면 자연스러운 확장
2. **spec.md를 PR에 포함** — 코드 변경 ↔ 명세 변경 동시 리뷰
3. **테스트를 명세에 인용** — `AC: 통합테스트 X 통과` 형태
4. **모호함 발견 시 /clarify 강제** — AI에 바로 던지기 전 한 번 더 정밀화
5. **CI에 명세-코드 일관성 검증** — 도구가 미성숙하므로 사내 스크립트로라도
...
- Spec-Driven Development 소개 글: https://github.blog/ai-and-ml/generative-ai/spec-driven-development-with-ai-get-started-with-a-new-open-source-toolkit/
- Amazon Kiro: https://kiro.dev/
- API-First 가이드: https://www.openapis.org/
